...
class MetricParser:
    ...
    @classmethod
    def getIamDetails(self, cpu_util_df, resource, instance_id):
        try:
            filter = {
                'Name': 'instance-id',
                'Values': [instance_id]
    ...
    @classmethod
    def getTagDetails(self, cpu_util_df, resource, instance_id):
        try:         
            tag_details = []
            filter = {
                'Name': 'resource-id', 
    ...
    @classmethod
    def getSecurityGroupDetails(self, cpu_util_df, instance, instance_id):
        security_gp_name = []
    ...
    @classmethod
    def getInstanceStatus(self, cpu_util_df, instance, instance_id ):
        try:
    ...
    @classmethod
    def getInstanceDetails(self, cpu_util_df, instance, instance_id ):
    ...
    @classmethod
    def buildQuery(self, client, instance_id, period, start_time, end_time, metricVar, stat='Maximum'):
    ...
    @classmethod
    def Metricutilization(self, cpu_util_df, response, instance_id,column_name):
        network_value = {}
        ...
        try:
            ...
            for region in regions:
                ...
                for instance in ec2_resource.instances.all():
                    ...
                    collector.getTagDetails(cpu_util_df, ec2_client, instance_id)
                    collector.getInstanceDetails(cpu_util_df, instance, instance_id)
                    collector.getInstanceStatus(cpu_util_df, instance, instance_id)
                    collector.getSecurityGroupDetails(cpu_util_df, instance, instance_id)
                    collector.getIamDetails(cpu_util_df, ec2_client, instance_id)
                    ...
                    period = 900
                    response = cw_collector.buildQuery(client, instance_id, period,start_time, end_time,'CPUUtilization')
                    #print(response)
                    #
                    # Collecting Response from the Cloud Watch
                    network_in_response = cw_collector.buildQuery(client, instance_id, period,start_time, end_time,'NetworkIn', )
                    network_out_response = cw_collector.buildQuery(client, instance_id, period,start_time, end_time,'NetworkOut', )
                    diskRead_response = cw_collector.buildQuery(client, instance_id, period,start_time, end_time,'DiskReadBytes')
                    diskWrite_response = cw_collector.buildQuery(client, instance_id, period,start_time, end_time,'DiskWriteBytes')
                    status_response = cw_collector.buildQuery(client, instance_id, period,start_time, end_time,'StatusCheckFailed')
